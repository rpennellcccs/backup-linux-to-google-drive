 ...
 * author:      ben servoz (slightly modified by Sergio Viudes: http://sviudes.blogspot.com.es/2014/07/scripts-para-crear-y-subir-backups-de.html)
...
$chunk_size = 256 * 1024 * 4 ; // this will upload files 1MB at a time
//   miscellaneous
$verbose = true ;	
...
function parse_response( $raw_data ) {
	...
	for( $i=0 ; $i<count($raw_data) ; $i++ ) {
		...
		if ( strpos($raw_datum, "HTTP") === 0 ) {
        		$parsed_response['code'] = explode( " ", $raw_datum ) ;
		...
		} else if( count($parsed_response['headers']) > 0 ) {
